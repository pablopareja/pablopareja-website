...
const Header: FC<HeaderProps> = ({ black }: HeaderProps) => {
  ...
  const isWhiteBackground = useAppSelector(
    (state) => state.common.isWhiteBackground
  );
    ...
    <div>
      <ClientOnly>
        {useMd({ includeBiggerScreens: true }) && (
          <nav
            className={cx({
              "relative md:px-32 md:py-16 flex justify-between w-full text-xs font-sans z-10":
                true,
              "text-black": black,
              "text-white": !black,
            })}
          ...
          <>
            ...
            {!pathname.includes("contact") && (
              <Link
                href={`/contact?lang=${lang}`}
                className={cx({
                  "fixed z-10 p-2 text-xs leading-4 border bottom-4 right-4 ml-14":
                    true,
